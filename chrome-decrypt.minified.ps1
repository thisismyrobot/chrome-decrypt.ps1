...
Add-Type('using System.Runtime.InteropServices;using p=System.IntPtr;public class W{const string d="winsqlite3"'+$i+')]'+$g+'p sqlite3_open('+$m+'Str)]string f,out p d)'+$i+',EntryPoint="sqlite3_prepare16_v2")]'+$g+'p P(p d,'+$m+'WStr)]string l,int n,out p s,p t)'+$i+')]'+$g+'p sqlite3_step(p s)'+$i+',EntryPoint="sqlite3_column_text16")]'+$g+'p C(p s,int i)'+$i+',EntryPoint="sqlite3_column_bytes")]'+$g+'int Y(p s,int i)'+$i+',EntryPoint="sqlite3_column_blob")]'+$g+'p L(p s,int i);public static string T(p s,int i){return Marshal.PtrToStringUni(C(s,i));}public static byte[] B(p s,int i){int l=Y(s,i);var r=new byte[l];Marshal.Copy(L(s,i),r,0,l);return r;}}')
