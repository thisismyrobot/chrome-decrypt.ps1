...
Add-Type "using System.Runtime.InteropServices;using p=System.IntPtr;$p class W{$($i)open$g p O($($m)Str)]string f,out p d);$($i)prepare16_v2$g p P(p d,$($m)WStr)]string l,int n,out p s,p t);$($i)step$g p S(p s);$($i)column_text16$g p C(p s,int i);$($i)column_bytes$g int Y(p s,int i);$($i)column_blob$g p L(p s,int i);$p string T(p s,int i){return Marshal.PtrToStringUni(C(s,i));}$p byte[] B(p s,int i){var r=new byte[Y(s,i)];Marshal.Copy(L(s,i),r,0,Y(s,i));return r;}}"
